...
class ControllButton{
  ...
  float iconX;
  float iconY;
  int buttonColor = color(200,200,200);
  ...
  public void move(float pa,float a){
    if(boolDrag == true){
  ...
  public void checkPressed(float a, float b){
...
class testButton{
 ...
 public void fileSelected(File selection) {
  ...
  if (selection == null) {
    ...
    saveFile = new File(sketchPath("")+"/data/MFA/"+selection.getName());
    saveStream(saveFile.getAbsolutePath(),selection.getAbsolutePath());
    frameCount = -1;
  }
}
...
class LoadButton{
 ...
 public void clickButton(float x,float y){
   if(x>positionX&&x<positionX+buttonLength&&y>positionY&&y<positionY+buttonHeight){
     ac.stop();
     selectInput("Select a music file:", "fileSelected",selection);
   }
 }
